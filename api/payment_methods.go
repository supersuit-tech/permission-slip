...
func handleCreateSetupIntent(deps *Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		custID, err := db.GetStripeCustomerIDByUser(r.Context(), deps.DB, profile.ID)
		if err != nil {
			log.Printf("[%s] CreateSetupIntent: stripe customer lookup: %v", TraceID(r.Context()), err)
			CaptureError(r.Context(), err)
			RespondError(w, r, http.StatusInternalServerError, InternalError("Failed to load profile"))
			return
		}
		...
		if custID != nil && *custID != "" {
			...
			if err := db.SetStripeCustomerID(r.Context(), deps.DB, profile.ID, stripeCustomerID); err != nil {
				log.Printf("[%s] CreateSetupIntent: save customer ID: %v", TraceID(r.Context()), err)
				CaptureError(r.Context(), err)
				RespondError(w, r, http.StatusInternalServerError, InternalError("Failed to save payment customer"))
				return
			}
		}
