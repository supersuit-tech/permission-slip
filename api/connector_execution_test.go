...
func TestResolveCredentialsWithFallback_OAuthFallsBackToAPIKey(t *testing.T) {
	...
	creds, err := resolveCredentialsWithFallback(t.Context(), deps, uid, connID+".do_thing", reqCreds)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestResolveCredentialsWithFallback_BothFail_ReturnsError(t *testing.T) {
	...
	_, err = resolveCredentialsWithFallback(t.Context(), deps, uid, connID+".do_thing", reqCreds)
	if err == nil {
