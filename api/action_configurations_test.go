...
func TestCreateActionConfig_WithCredential(t *testing.T) {
	...
	testhelper.InsertCredential(t, tx, credID, uid, "test-service")
	...
	body := fmt.Sprintf(`{
		...
		"action_type": %q,
		"credential_id": %q,
		"parameters": {"channel": "#releases"},
		"name": "Post to releases"
	}`, agentID, connID, actionType, credID)
...
func TestCreateActionConfig_OtherUsersCredentialRejected(t *testing.T) {
	t.Parallel()
	tx, uid, agentID, connID, actionType := setupActionConfigTest(t)
	...
	otherCredID := testhelper.GenerateID(t, "cred_")
	testhelper.InsertCredential(t, tx, otherCredID, uid2, "other-service")
	...
	body := fmt.Sprintf(`{
		...
		"credential_id": %q,
		"name": "Should fail"
	}`, agentID, connID, actionType, otherCredID)
	...
	if w.Code != http.StatusBadRequest {
		t.Fatalf("expected 400 for other user's credential, got %d: %s", w.Code, w.Body.String())
	}
}

func TestUpdateActionConfig_OtherUsersCredentialRejected(t *testing.T) {
	t.Parallel()
	tx, uid, agentID, connID, actionType := setupActionConfigTest(t)
	...
	body := fmt.Sprintf(`{"credential_id": %q}`, otherCredID)
	...
	if w.Code != http.StatusBadRequest {
		t.Fatalf("expected 400 for other user's credential on update, got %d: %s", w.Code, w.Body.String())
	}
}
