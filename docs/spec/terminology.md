...
### Registration Invite
A single-use, time-limited code generated by a user from the Permission Slip dashboard to authorize an agent to register. The invite code is embedded in an invite URL (e.g., `https://your-server.example.com/invite/PS-R7K3-X9M4`). The user shares this URL with the agent, and the agent POSTs directly to it to register.
...
- Self-describing with optional `name` and `description` — no linked configuration record required
- Revocable instantly from the web UI at any time
- Account scope (`connector_instance_id`) is editable in place from the connector config page — no revoke-and-recreate required
...
### Standing Approval Template
A connector-authored preset that recommends a standing approval users can apply from the connector config page. Templates are synced from connector manifests at startup and are purely a convenience — agents can request any action on an enabled connector without applying a template first.

**Key characteristics:**
- Scoped to a connector and action type with pre-filled constraints
- Applied via `POST /standing-approval-templates/{id}/apply` to create a standing approval for an agent
- Optional `duration_days` suggests an expiry when applied (user can override)

See [ADR-010](../adr/010-remove-action-configurations.md) for the migration from action configuration templates.
...
| **Standing Approval Template** | Connector-authored preset for creating standing approvals from the web UI |
