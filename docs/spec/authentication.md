...
This document specifies how agents establish and prove their identity when communicating with the Permission Slip service.

> **Architecture Note:** In the Permission Slip architecture, agents authenticate exclusively with the Permission Slip service — not with external services (Gmail, Stripe, etc.) directly. Permission Slip is the only party that verifies agent signatures and manages agent registrations. References to "service" or "server" in this document refer to the Permission Slip service unless otherwise noted.

---

## Cryptographic Identity

Every agent in the Permission Slip protocol is identified by a cryptographic key pair. The agent's public key is registered with Permission Slip and serves as its verifiable identity, while the private key is used to sign requests and prove authenticity.
...
func deriveAgentID(publicKey ed25519.PublicKey) string {
	...
	fmt.Println(agentID)
	// agent_x7K9mP4nQ8rT2vW5yZ1aC3bD6eF9gH0jK4lM7nP0qR3
...
```
X-Permission-Slip-Signature: agent_id="agent_x7K9mP4nQ8rT2vW5yZ1aC3bD6eF9gH0jK4lM7nP0qR3", algorithm="Ed25519", timestamp="1707667200", signature="dGhpc19pc19hX2Jhc2U2NHVybF9lbmNvZGVkX3NpZ25hdHVyZV93aXRoX25vX3BhZGRpbmc"
```
...
1. **Method:** `POST`
2. **Path:** `/invite/PS-R7K3-X9M4`
3. **Query string:** `debug=true` (already canonical in this case)
4. **Timestamp:** `1707667200`
5. **Body hash:** Canonicalize the JSON body using RFC 8785 (JCS), then compute SHA-256 hash and encode as lowercase hex. For this example body `{"public_key":"ssh-ed25519 AAAAC3Nza...","request_id":"6f1a7c30-9b2e-4d91-8c3f-2a4b6c7d8e9f"}`, this produces: `a1b2c3d4e5f6...` (example hash)
...
3. **Agent lookup:**
   ...
   - For `POST /invite/{invite_code}` requests (agent registration): Use the `public_key` provided in the request body
