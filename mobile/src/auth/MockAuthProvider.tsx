...
import type { AuthState, AuthResult, AppSession, AppUser } from "./types";
 ...
 * can test the full app UI without a running backend. The provider
...
export function MockAuthProvider({ children }: { children: ReactNode }) {
  useEffect(() => {
    console.log(
      "[MockAuth] Running in mock auth mode — real auth is bypassed. " +
        "Set EXPO_PUBLIC_MOCK_AUTH= (empty) to disable.",
    );
  }, []);

  const noOp = useCallback(async (): Promise<AuthResult> => ({ error: null }), []);
