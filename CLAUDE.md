...
- After making the first commit on a branch, do NOT automatically create a pull request. Instead, provide a pre-filled GitHub PR creation link so the user can create it themselves. Format as a clickable markdown link: `[Create PR](https://github.com/supersuit-tech/permission-slip/compare/main...BRANCH?expand=1&title=URL_ENCODED_TITLE&body=URL_ENCODED_BODY)` — include a suggested title and body with a summary of changes. For subsequent commits, push to the existing PR branch.
