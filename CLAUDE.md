...
- After making the first commit on a branch, check if a PR already exists for the current branch by running `GH_HOST=github.com GH_REPO=supersuit-tech/permission-slip gh pr view --json url -q .url 2>/dev/null`. If a PR exists, use the returned URL (do NOT show the create-PR link). If no PR exists, do NOT automatically create one — instead, provide a pre-filled GitHub PR creation link so the user can create it themselves. Format: `Click here to create PR: https://github.com/supersuit-tech/permission-slip/compare/main...BRANCH?expand=1&title=URL_ENCODED_TITLE&body=URL_ENCODED_BODY` — include a suggested title and body with a summary of changes. For subsequent commits, push to the existing PR branch.
