...
export type ApplyStandingApprovalTemplateResponse =
  components["schemas"]["ApplyStandingApprovalTemplateResponse"];

export function useApplyStandingApprovalTemplate() {
  ...
  const mutation = useMutation({
    mutationFn: async (input: { templateId: string; agentId: number }) => {
      ...
      const { data, error } = await client.POST(
        "/v1/standing-approval-templates/{id}/apply",
        {
