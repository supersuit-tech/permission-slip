...
|--------|----------|-------------|
| `imessage.list_chats` | No | Recent conversations with optional `unread_only`, `since`/`before` (filters on `last_message_at` client-side), `order_by`/`sort` (client-side ordering by `last_activity` or `contact_name`), and per-chat `unread_count` (when imsg supports it). Standing approvals can constrain `$data_window`, `order_by`, `sort`, and `limit` to auto-approve recent chats only. |
