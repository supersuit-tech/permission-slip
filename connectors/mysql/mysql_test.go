...
func TestCheckTableAllowed(t *testing.T) {
	...
	tests := []struct {
		...
		{"case insensitive", "Users", []string{"users"}, false},
		{"not allowed", "secrets", []string{"users", "orders"}, true},
		{"empty allowlist", "anything", nil, false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("checkTableAllowed() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
...
func TestCheckColumnsAllowed(t *testing.T) {
	...
	tests := []struct {
		...
		{"all allowed", []string{"name", "email"}, []string{"name", "email", "id"}, false},
		{"case insensitive", []string{"Name"}, []string{"name"}, false},
		{"not allowed", []string{"password"}, []string{"name", "email"}, true},
		{"empty allowlist", []string{"anything"}, nil, false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("checkColumnsAllowed() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
