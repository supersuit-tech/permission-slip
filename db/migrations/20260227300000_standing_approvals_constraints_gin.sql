...
-- CONCURRENTLY avoids holding an exclusive lock on the table during creation.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_standing_approvals_constraints_gin
    ON standing_approvals USING gin (constraints jsonb_path_ops)
    WHERE constraints IS NOT NULL;

-- +goose Down
DROP INDEX CONCURRENTLY IF EXISTS idx_standing_approvals_constraints_gin;
