		...
		func(value string, i int) error {
			_, err := tx.Exec(context.Background(),
				`INSERT INTO approvals (approval_id, agent_id, approver_id, action, context, status, execution_status, executed_at, expires_at)
				 VALUES ($1, $2, $3, '{"type":"test"}', '{"description":"test"}', 'approved', $4, strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+1 hour'))`,
				fmt.Sprintf("%s_%d", base, i), agentID, uid, value)
			return err
		})
}
...
func TestApprovalExecutionColumnsConsistencyConstraint(t *testing.T) {
	...
	_, err := tx.Exec(ctx,
		`INSERT INTO approvals (approval_id, agent_id, approver_id, action, context, status, expires_at)
		 VALUES ($1, $2, $3, '{"type":"test"}', '{"description":"test"}', 'pending', strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+1 hour'))`,
		id1, agentID, uid)
	if err != nil {
	...
	_, err = tx.Exec(ctx,
		`INSERT INTO approvals (approval_id, agent_id, approver_id, action, context, status, execution_status, executed_at, expires_at)
		 VALUES ($1, $2, $3, '{"type":"test"}', '{"description":"test"}', 'approved', 'success', strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+1 hour'))`,
		id2, agentID, uid)
	if err != nil {
	...
	_, err = tx.Exec(ctx,
		`INSERT INTO approvals (approval_id, agent_id, approver_id, action, context, status, execution_status, expires_at)
		 VALUES ($1, $2, $3, '{"type":"test"}', '{"description":"test"}', 'approved', 'success', strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+1 hour'))`,
		id3, agentID, uid)
	if err == nil {
	...
	_, err = tx.Exec(ctx,
		`INSERT INTO approvals (approval_id, agent_id, approver_id, action, context, status, executed_at, expires_at)
		 VALUES ($1, $2, $3, '{"type":"test"}', '{"description":"test"}', 'approved', strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+1 hour'))`,
		id4, agentID, uid)
	if err == nil {
